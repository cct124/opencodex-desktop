...
从 0.1.4 起，本 fork 的桌面壳位于 `desktop-fork/`；`desktop/` 保留上游新加入的桌面实现，便于以后同步和评估。`Desktop installers` 只构建本 fork 的桌面壳；安装包身份、用户数据目录、托盘三种原生切换操作和固定端口保持兼容。上游模型目录已加入 GPT-6 Sol / Luna。
...
桌面版从 **0.1.0** 起独立维护：同步修改 `desktop-fork/package.json` 和 `desktop-fork/src-tauri/Cargo.toml`，然后执行 `cargo check --offline --manifest-path desktop-fork/src-tauri/Cargo.toml` 更新 `Cargo.lock`。Tauri 读取桌面 package 的版本，构建和测试会拒绝不一致。后端的根 `package.json` 保留上游版本；控制页同时显示两个版本，原管理界面的版本徽标仍表示 OpenCodex 后端。
...
```powershell
& .\node_modules\bun\bin\bun.exe install --cwd desktop-fork --frozen-lockfile
cargo fetch --locked --manifest-path desktop-fork/src-tauri/Cargo.toml
& .\node_modules\bun\bin\bun.exe run desktop-fork/scripts/build-installer.ts
& .\node_modules\bun\bin\bun.exe run desktop-fork/scripts/package-smoke.ts
```

输出为 `desktop-fork/src-tauri/target/release/bundle/nsis/OpenCodex Desktop_<version>_x64-setup.exe` 和同名 `.sha256`。固定 Tauri CLI、Rust 锁文件和 Bun 版本；生成脚本从允许列表复制资源、安装生产依赖并收集许可文件，不携带工作区配置、日志或开发笔记。`desktop-manifest.json` 用于构建与验收时检查资源哈希；程序启动时检查必需文件和版本，不将该清单宣称为数字签名。

Mac 上安装 Xcode Command Line Tools、Rust 1.92.0 和根 package 指定的 Bun，执行相同的 Bun/Rust 命令（将 Windows 的 Bun 路径替换为 `bun`）。必须在目标原生架构上构建，输出 `desktop-fork/src-tauri/target/release/bundle/dmg/OpenCodex Desktop_<version>_<aarch64|x64>.dmg`；最低 macOS 13。打开 DMG，将完整应用拖入 Applications。构建采用 ad-hoc 签名，未作 Apple 开发者签名或公证，因此 macOS 可能阻止首次打开，需要在系统隐私与安全设置中明确允许。完整包验收还应运行：

```sh
codesign --verify --deep --strict "desktop-fork/src-tauri/target/release/bundle/macos/OpenCodex Desktop.app"
bun run desktop-fork/scripts/package-smoke.ts "desktop-fork/src-tauri/target/release/bundle/macos/OpenCodex Desktop.app/Contents/Resources/runtime"
```

统一收集到 `desktop-fork/.bundle/artifacts/` 的产物只属于本次构建。Mac 包含原架构的 Bun 和原生依赖，保留可执行权限，不携带 npm 命令符号链接。
...
构建后双击 `desktop-fork/src-tauri/target/debug/opencodex-desktop.exe` 启动持久化模式。首次启动先配置提供方；从托盘打开“桌面状态与控制”，确认显示的 Codex 目录，点击“启用 Codex 代理并记住选择”。首次连接会重启一次后端，之后恢复与接回无需重启代理。
...
- `connection.json`：连接选择、目标 Codex 目录、持久监听端口 `listenPort` 和用于异常恢复的运行记录。
- `runs/`：每次应用启动的日志与状态。
- `webview/`：管理窗口的浏览器数据。
...
从 **0.1.2** 起，持久化模式首次选择可用回环端口并保存，之后重启、退出再打开、配置导入和崩溃恢复均复用它。升级旧版时优先沿用连接记录中上次代理的端口。端口被占用或无法监听时明确报错并停止启动，保留连接选择和 Codex 配置，不自动换端口或接管占用者。正常重启代理后，Codex 可继续使用原地址，中断的请求重试即可；若升级前的任务已缓存其他旧地址，需先完整重启一次 Codex。开发预览仍使用临时端口。
...
无窗口的 Windows 连接回归可从仓库根目录运行 `cargo run --locked --offline --manifest-path desktop-fork/src-tauri/Cargo.toml --example runtime-probe -- persistent`。它复用正式应用的 Windows 进程启动逻辑，使用 `.tmp/desktop` 下的独立客户端夹具，并将结果写入 `.tmp/desktop/runtime-probe-persistent.log`。`package` 模式验证已暂存的安装包资源。登录 Windows 桌面后，可运行 `cargo test --locked --offline --manifest-path desktop-fork/src-tauri/Cargo.toml windows_runtime::tests::child_runs_without_package_identity -- --ignored --exact` 验证子进程环境；它会主动执行两个子进程用例。这三个用例标记为 ignored，以免无桌面 Shell 的 CI 会话误执行交互环境检查。

构建 debug 程序后，可在 `desktop-fork/` 中执行：
...
| `desktop-fork/` | 放置 Tauri 窗口、托盘、进程管理及桌面打包代码 |
