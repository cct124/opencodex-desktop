...
const metadata = Bun.spawnSync(["cargo", "metadata", "--locked", "--offline", "--format-version", "1", "--manifest-path", join(repo, "desktop-fork/src-tauri/Cargo.toml")], { stdout: "pipe", stderr: "pipe", windowsHide: true });
...
const bunNotices = join(repo, "desktop-fork/licenses", `bun-${Bun.version}`);
...
writeFileSync(join(repo, "desktop-fork/.bundle/icon.ico"), Buffer.concat([icon, png]));
