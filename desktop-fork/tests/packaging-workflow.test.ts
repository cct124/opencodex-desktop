...
test("macOS bundle keeps a PNG for Tauri's compiled default window/tray icon", () => {
  const config = JSON.parse(readFileSync(resolve(repo, "desktop-fork/src-tauri/tauri.macos.bundle.conf.json"), "utf8"));
  ...
  expect(existsSync(resolve(repo, "desktop-fork/src-tauri", png))).toBe(true);
});

test("desktop build metadata stays separate from the proxy release line", () => {
  const versions = packageVersions(repo);
  const config = JSON.parse(readFileSync(resolve(repo, "desktop-fork/src-tauri/tauri.conf.json"), "utf8"));
  expect(resolve(repo, "desktop-fork/src-tauri", config.version)).toBe(resolve(repo, "desktop-fork/package.json"));
  const lock = Bun.TOML.parse(readFileSync(resolve(repo, "desktop-fork/src-tauri/Cargo.lock"), "utf8")) as { package: { name: string; version: string }[] };
...
test("installer automation builds three native platforms and only version tags grant release writes", () => {
  ...
  expect(upload.with.path).toBe("desktop-fork/.bundle/artifacts/");
