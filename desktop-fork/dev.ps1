...
if (-not (Test-Path -LiteralPath $taskBun)) { throw '缺少项目内 Bun，请先按 desktop-fork/README.md 完成基线依赖安装。' }
...
& $taskCargo run --locked --manifest-path (Join-Path $PSScriptRoot 'src-tauri/Cargo.toml') -- --preview
exit $LASTEXITCODE
