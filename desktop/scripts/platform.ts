...
export function packageVersions(repo: string) {
  ...
  const desktop = JSON.parse(readFileSync(join(repo, "desktop/package.json"), "utf8"));
  const cargo = Bun.TOML.parse(readFileSync(join(repo, "desktop/src-tauri/Cargo.toml"), "utf8")) as { package: { version: string } };
  if (!/^\d+\.\d+\.\d+$/.test(desktop.version) || cargo.package.version !== desktop.version) {
    throw new Error("Use matching desktop/package.json and Cargo.toml versions (major.minor.patch)");
  }
