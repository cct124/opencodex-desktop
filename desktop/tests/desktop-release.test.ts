...
function fixture() {
  ...
  mkdirSync(join(repo, "desktop/releases"));
  writeFileSync(join(repo, "desktop/package.json"), JSON.stringify({ version: "0.1.1" }));
  ...
  writeFileSync(join(repo, "desktop/releases/0.1.1.md"), "Desktop preview notes\n");
...
test("mismatched, incomplete or modified artifacts fail before any release write", async () => {
  for (const failure of ["missing-platform", "wrong-commit", "wrong-version", "modified-installer", "extra-file", "missing-notes"]) {
    ...
    if (failure === "missing-notes") rmSync(join(f.repoRoot, "desktop/releases/0.1.1.md"));
