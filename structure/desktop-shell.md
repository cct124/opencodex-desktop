...
This repository retains the upstream `desktop/` implementation for synchronization. The fork's
shipping shell lives in `desktop-fork/`: `.github/workflows/desktop-build.yml` builds that shell
and publishes only `desktop-v*` releases. Its application identity, persistent data directory,
saved port, close-to-tray behavior and native restore/reconnect controls remain the fork's own.
The runtime package includes `desktop-fork/runtime/entry.ts` and the shared proxy and dashboard;
it does not install or execute the upstream desktop shell. See `desktop-fork/README.md` for
the fork's lifecycle and upgrade contract. The sections below describe the retained upstream shell.
